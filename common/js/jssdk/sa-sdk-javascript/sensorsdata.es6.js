...
sd.detectMode = function(){
    ...
    var vtrackMode = {
      isVtrackMode: function() {
        return location.search.match(/sa-visual-mode=true/) || (_.sessionStorage.isSupport() && typeof sessionStorage.getItem('sensors-visual-mode') === 'string');
      },
      loadVtrack: function() {
        _.loadScript({
          success: function() {
  ...
  //    sd.noticePluginIsReady();
    ...
    if(heatmapMode.isSeachHasKeyword()){
      ...
      // 通过检查iframe
    }else if (window.parent !== self && vtrackMode.isVtrackMode()) {
...
sd.modules['SiteLinker'] = (function () {
  ...
  siteLinker.getCurrenId = function() {
    ...
    if (this._.urlSafeBase64 && this._.urlSafeBase64.encode) {
      distinct_id = distinct_id ? this._.urlSafeBase64.encode(_.base64Encode(distinct_id)) : '';
